...
impl ThreadPool {
    ...
    pub fn add_perm_threads(&self, num_threads: usize) {
        ...
        for thread_id in 0..num_threads {
            ...
            thread::spawn(move || ThreadPool::perm_thread_worker(thread_id, thread_sender, thread_receiver));
    ...
    fn receive_threads(pool: &mut ThreadPoolInner) {
    ...
    where F: FnOnce() -> T, F: Send + 'static, T: Send + 'static
    {
        ...
        ThreadPool::receive_threads(&mut pool);
    ...
    pub fn result<T>(&self, r: Receiver<T>) -> T {
        ...
        loop {
            ...
            if let Ok(result) = r.try_recv() {
                // Alert threadpool that we are exiting
                thread_sender.send(ThreadStatus::Exited);
                return result;
            }
